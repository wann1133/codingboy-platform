...
const parseFeatureString = (raw: unknown): string => {
  ...
  if (typeof raw === 'string') {
    try {
      ...
      // ignore parse issues and fallback to raw string
    }
...
const mapPortfolioResponseToForm = (payload: unknown): PortfolioFormState => {
  const fallback = defaultPortfolioForm();
  ...
  return {
    ...
    category: typeof item.category === 'string' ? item.category : fallback.category,
    ...
    client: typeof item.client === 'string' ? item.client : fallback.client,
    testimonial: typeof item.testimonial === 'string' ? item.testimonial : fallback.testimonial,
    rating:
      typeof item.rating === 'number'
        ? String(item.rating)
        : typeof item.rating === 'string' && item.rating.trim().length > 0
        ? item.rating
        : fallback.rating,
    features: parseFeatureString(item.features) || fallback.features,
...
const EditPortfolioPage = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  useEffect(() => {
    const fetchPortfolio = async () => {
      try {
        ...
        setForm(mapPortfolioResponseToForm(data.portfolio));
      } catch (error) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        setErrorMessage(data?.error ?? 'Gagal memperbarui portofolio.');
        return;
      }

      setNotice('Showcase berhasil diperbarui. Mengarahkan ke daftar portofolio...');
      setTimeout(() => {
      ...
      <div className="mx-auto flex max-w-4xl flex-col gap-8 px-6">
        <header className="flex flex-col gap-4">
          ...
          <div>
            ...
            <h1 className="mt-4 text-3xl font-semibold text-white">Edit showcase</h1>
            <p className="mt-2 max-w-2xl text-sm text-slate-400">
              Perbarui studi kasus dengan highlight stack dan status proyek.
            </p>
          </div>
        </header>
